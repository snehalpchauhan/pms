...
export async function registerRoutes(
  ...
  /** Open VoiceLink (or compatible) from PMS chat — chat stays in PMS; call opens in a new tab. */
  const voiceLinkBodySchema = z.object({
  ...
  app.post("/api/chat/voice-link", requireAuth, async (req, res) => {
    const currentUser = req.user as any;
    const parsed = voiceLinkBodySchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ message: parsed.error.issues[0]?.message ?? "Invalid body" });
    }
    ...
    const roomKey =
      channel.projectId != null ? `pms-p${channel.projectId}-c${channelId}` : `pms-c${channelId}`;
      ...
      try {
        ...
        if (!r.ok) {
          ...
          return res.status(502).json({
            message: "VoiceLink service returned an error",
            detail: text.slice(0, 400),
          });
        }
        const data = (await r.json()) as { url?: string; joinUrl?: string };
        const url = data.joinUrl ?? data.url;
        if (!url || typeof url !== "string") {
          return res.status(502).json({ message: "VoiceLink service did not return url or joinUrl" });
        }
        return res.json({ url });
      } catch (e) {
